...
#define UART_ISR_EN_TNFUL \
    (0b1 << 11) // Transmitter FIFO Nearly Full interrupt (enable, clears mask=0)
...
#define UART_MODEM_CTRL_RTS_FL1 \
    (0b0 << 1) // Request to send output control (EMIOUARTxRTSN output forced to logic 1)
#define UART_MODEM_CTRL_RTS_FL0 \
    (0b1 << 1) // Request to send output control (EMIOUARTxRTSN output forced to logic 0)
...
#define UART_MODEM_STATUS_DCD_H \
    (0b0 << 7) // Data Carrier Detect (DCD) input signal from PL(EMIOUARTxDCDN) status (input is
               // high)
#define UART_MODEM_STATUS_DCD_L \
    (0b1 << 7) // Data Carrier Detect (DCD) input signal from PL(EMIOUARTxDCDN) status (input is
               // low)
#define UART_MODEM_STATUS_RI_H \
    (0b0 << 6) // Ring Indicator (RI) input signal from PL(EMIOUARTxRIN) status (input is high)
#define UART_MODEM_STATUS_RI_L \
    (0b1 << 6) // Ring Indicator (RI) input signal from PL(EMIOUARTxRIN) status (input is low)
#define UART_MODEM_STATUS_DSR_H \
    (0b0 << 5) // Data Set Ready (DSR) input signal from PL(EMIOUARTxDSRN) status (input is high)
#define UART_MODEM_STATUS_DSR_L \
    (0b1 << 5) // Data Set Ready (DSR) input signal from PL(EMIOUARTxDSRN) status (input is low)
#define UART_MODEM_STATUS_CTS_H \
    (0b0 << 4) // Clear to Send (CTS) input signal from PL(EMIOUARTxCTSN) status (input is high)
#define UART_MODEM_STATUS_CTS_L \
    (0b1 << 4) // Clear to Send (CTS) input signal from PL(EMIOUARTxCTSN) status (input is low)
#define UART_MODEM_STATUS_DDCD (0b1 << 3) // Delta Data Carrier Detect status (change has occurred)
#define UART_MODEM_STATUS_TERI \
    (0b1 << 2) // Trailing Edge Ring Indicator status (Trailing edge has occurred)
...
#define UART_FLOW_CTRL_DL_DIS \
    (0) // RxFIFO trigger level for Ready To Send (RTS)output signal (EMIOUARTxRTSN) de-assertion
        // (0-3 disable)
